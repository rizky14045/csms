...
<table>
    <tr>
        <td colspan="{{ $cols }}" style="text-align:center; font-size:14pt; font-weight:bold;">
            {{ $isFa ? 'FINAL ASSESSMENT (FA)' : 'SELF ASSESSMENT (SA)' }} KPI KEAMANAN
        </td>
    </tr>
    <tr>
        <td colspan="{{ $cols }}" style="text-align:center; font-weight:bold;">
            {{ $kpi->unit->name ?? '-' }} - Tahun {{ $kpi->year }} - {{ $kpi->period_label }}
        </td>
    </tr>
    ...
    @foreach ($areas as $area)
        ...
        @foreach ($area['sub_areas'] as $subIdx => $subArea)
            @php
                ...
                $hasilSa = count($levels) > 0 ? round($uploaded / count($levels), 4) : 0;
    ...
    <tr>
        <td colspan="{{ $cols - 1 }}" style="{{ $td }} font-weight:bold; background-color:#DDEBF7;">Total Score ML Self Assessment (SA)</td>
