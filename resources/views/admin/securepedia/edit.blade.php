...
<div class="py-3 d-flex align-items-center gap-2">
    ...
    <h4 class="mb-0">Edit Data Securepedia</h4>
</div>
        ...
        <div class="card shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom py-3">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0 fw-semibold">Edit Data Securepedia</h6>
                </div>
            </div>
            <div class="card-body p-4">
                <form action="{{route('admin.securepedia.update',['securepedia'=>$securepedia->id])}}" enctype="multipart/form-data" method="POST"
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('title') is-invalid @enderror" type="text" id="title"
                            required placeholder="Masukan judul" name="title" value="{{old('title', $securepedia->title)}}">
                        @error('title')
                    ...
                    <div class="form-group mb-4">
                        ...
                        @if($securepedia->file)
                        <div class="mb-2">
                            <a href="{{ asset('uploads/securepedia/' . $securepedia->file) }}" target="_blank"
                                class="btn btn-sm btn-outline-primary">
                                <i data-feather="eye" style="width:13px;height:13px;margin-right:4px;"></i>Lihat File Saat Ini
                            </a>
                        </div>
                        @endif
                        <input class="form-control @error('file') is-invalid @enderror" type="file" id="file"
                            accept=".pdf,.doc,.docx" name="file">
                        <div class="form-text text-muted">Kosongkan jika tidak ingin mengganti file. Format: PDF, DOC, DOCX</div>
                        @error('file')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
