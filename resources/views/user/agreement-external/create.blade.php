        ...
        <div class="card shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom py-3">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0 fw-semibold">Tambah Data Perjanjian Kerjasama Eksternal</h6>
                </div>
            </div>
                ...
                <form action="{{ route('user.agreement-external.store') }}" method="POST" id="form-agreement"
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('pkt_number') is-invalid @enderror" type="text" id="pkt_number"
                            required placeholder="Masukan nomor PKT" name="pkt_number" value="{{ old('pkt_number') }}">
                        @error('pkt_number')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('pkt_title') is-invalid @enderror" type="text" id="pkt_title"
                            required placeholder="Masukan judul PKT" name="pkt_title" value="{{ old('pkt_title') }}">
                        @error('pkt_title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('expired_date') is-invalid @enderror" type="date" id="expired_date"
                            required name="expired_date" value="{{ old('expired_date') }}">
                        @error('expired_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
