...
<div class="py-3 d-flex align-items-center gap-2">
    ...
    <h4 class="mb-0">Edit Satuan Pengamanan</h4>
</div>
        ...
        <div class="card shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom py-3">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0 fw-semibold">Edit Data Satuan Pengamanan</h6>
                </div>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('user.security.update', ['security' => $security->id]) }}" method="POST"
                    id="form-security" enctype="multipart/form-data"
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('name') is-invalid @enderror" id="name" type="text"
                            required placeholder="Masukan nama" name="name" value="{{ old('name', $security->name) }}">
                        @error('name')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('unit_work') is-invalid @enderror" id="unit_work" type="text"
                            required placeholder="Masukan unit kerja" name="unit_work"
                            value="{{ old('unit_work', $security->unit_work) }}">
                        @error('unit_work')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('nid') is-invalid @enderror" id="nid" type="text"
                            required placeholder="Masukan NID" name="nid" value="{{ old('nid', $security->nid) }}">
                        @error('nid')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('registration_number') is-invalid @enderror" id="registration_number"
                            ...
                            value="{{ old('registration_number', $security->registration_number) }}">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('expired_card_date') is-invalid @enderror" id="expired_card_date"
                            type="date" required name="expired_card_date"
                            value="{{ old('expired_card_date', $security->expired_card_date) }}">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('birth_place') is-invalid @enderror" id="birth_place" type="text"
                            required placeholder="Masukan tempat lahir" name="birth_place"
                            value="{{ old('birth_place', $security->birth_place) }}">
                        @error('birth_place')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('birth_date') is-invalid @enderror" id="birth_date" type="date"
                            required name="birth_date" value="{{ old('birth_date', $security->birth_date) }}">
                        @error('birth_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('last_education') is-invalid @enderror" id="last_education"
                            ...
                            value="{{ old('last_education', $security->last_education) }}">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('note') is-invalid @enderror" id="note" type="text"
                            placeholder="Masukan note" name="note" value="{{ old('note', $security->note) }}">
                        @error('note')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        <label for="kta_file" class="form-label">File KTA <span class="text-muted small">(opsional jika sudah ada)</span></label>
                        @if ($security->kta_file)
                            <div class="mb-2">
                                <a href="{{ asset('uploads/kta_files/' . $security->kta_file) }}" target="_blank"
                                    class="btn btn-sm btn-outline-info">
                                    <i data-feather="eye" style="width:14px;height:14px;" class="me-1"></i>Lihat KTA Saat Ini
                                </a>
                            </div>
                        @endif
                        <input class="form-control @error('kta_file') is-invalid @enderror" id="kta_file" type="file"
                            name="kta_file" accept=".pdf,.jpg,.jpeg,.png">
                        <div class="form-text text-muted">
                            Format: PDF, JPG, JPEG, PNG. Maksimal 5 MB.
                            @if ($security->kta_file) Kosongkan jika tidak ingin mengganti file KTA. @endif
                        </div>
                        @error('kta_file')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
