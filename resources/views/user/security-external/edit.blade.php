...
<div class="py-3 d-flex align-items-center gap-2">
    ...
    <h4 class="mb-0">Edit Personil Keamanan Eksternal</h4>
</div>
        ...
        <div class="card shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom py-3">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0 fw-semibold">Edit Data Personil Keamanan Eksternal</h6>
                </div>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('user.security-external.update', ['security' => $security->id]) }}" method="POST" id="form-security-external"
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('instansi') is-invalid @enderror" type="text" id="instansi"
                            required placeholder="Masukan instansi" name="instansi" value="{{ old('instansi', $security->instansi) }}">
                        @error('instansi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('regional_unit') is-invalid @enderror" type="text" id="regional_unit"
                            required placeholder="Masukan satuan wilayah" name="regional_unit" value="{{ old('regional_unit', $security->regional_unit) }}">
                        @error('regional_unit')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control @error('warrant_number') is-invalid @enderror" type="text" id="warrant_number"
                            required placeholder="Masukan nomor surat perintah" name="warrant_number" value="{{ old('warrant_number', $security->warrant_number) }}">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-select @error('note') is-invalid @enderror" name="note" id="note" required>
                            ...
                            <option value="Polri" {{ old('note', $security->note) == 'Polri' ? 'selected' : '' }}>Polri</option>
                            <option value="TNI" {{ old('note', $security->note) == 'TNI' ? 'selected' : '' }}>TNI</option>
                        </select>
                        @error('note')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
